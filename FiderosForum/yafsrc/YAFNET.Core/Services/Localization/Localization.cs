...
public class Localization : ILocalization
{
    ...
    public CultureInfo LoadTranslation(string fileName)
    {
        ...
        if (fileName.ToLower() != "french.json" && this.defaultLocale is null)
        {
#if !DEBUG
            ...
            this.defaultLocale = new Localizer(
                Path.Combine(BoardContext.Current.Get<BoardInfo>().WebRootPath, "languages", "french.json"),
                !isInstallPage);
#if !DEBUG
    ...
    public CultureInfo LoadTranslation()
    {
        ...
        if (this.TransPage.Equals("INSTALL"))
        {
            ...
            fileName = languageFile != null ? languageFile.CultureFile : "french.json";
        }
        else
        {
            if (BoardContext.Current.PageIsNull() || BoardContext.Current.PageUser.LanguageFile.IsNotSet()
